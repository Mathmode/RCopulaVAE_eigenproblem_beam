...
Script for Multivariate Gaussian VAE training
"""
...
# from MODULES.COPULAS.MVN_VAE import My_TMVN_VAE_withEigen 
from MODULES.TRAINING.full_multivariate_models import My_Bayesian_InverseForward_withEigen
...
def main():
    ...
    # Output Directory
    filename = f"22MarMVNlatent_{n_elements}Els_{n_modes}modes_Nosiy2.5_{lbound}lbound_Beta{beta}_{n_epochs}Epochs"
    ...
    model = My_Bayesian_InverseForward_withEigen(
        input_dim=input_dim, num_dofs=n_dofs, n_elements=n_elements,
        n_modes=n_modes, Ke_matrices=Ke_matrices, Mfree=Mfree,
        L_inv=L_inv, batch_size=batch_size, n_dims=n_dims, num_gaussians=num_gaussians,
        num_samples=num_samples, beta=beta, full_cov = full_cov, mean_f=mean_f,
        std_f=std_f, lbound=lbound, fixed_dofs_indices=fixed_dofs_indices
    )
    
    
    # model = My_TMVN_VAE_withEigen(
    #     input_dim=input_dim, num_dofs=n_dofs, n_elements=n_elements,
    #     n_modes=n_modes, Ke_matrices=Ke_matrices, Mfree=Mfree,
    ...
    model.compile(
        optimizer=optim, 
        loss=model.total_loss_with_regularizer,
        metrics=[model.Freqs_loss, model.MAC_modes_loss, model.total_loss_with_regularizer],
    ...
    GMM_props_test = inverse_model.predict(
        [Freqs_true_test, Rotmodes_true_test, Vertmodes_true_test, alpha_factors_true_test]
    )
    ...
    # Test_predicted_properties = {'test_means': test_means, 'test_scales': test_scales}
    Test_predicted_properties = {'GMM_props': GMM_props_test}
